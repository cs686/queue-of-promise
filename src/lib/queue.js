...
export function queueMixin(Queue) {
  ...
  Queue.prototype._init = function(max, options) {
    ...
    this._addItem = function(unit, stack, start, noAdd) {
      if (!(unit instanceof QueueUnit))
        throw new TypeError('"unit" is not QueueUnit');
      if (stack) {
        _queue.unshift(unit);
      } else {
        _queue.push(unit);
  ...
  function QueueUnit(fn, args, options) {
    ...
    if (!utils.isFunction(fn)) {
      throw new TypeError(
        "Queues only support function, '" + fn + "' is not function"
      );
    }
    this.fn = fn;
    this._errNo = 0;
  ...
  function getAddArgs(data, fn, con, each) {
    ...
    function fill(k) {
      var args = each
        ? utils.toArray([data[k]], [k], [data])
        : utils.toArray(data[k]);
      rdata[k] = [fn, args, con];
    }
    if (isArray) {
  ...
  Queue.prototype.addArray = function(array, start, jump) {
    ...
    for (var i = 0; i < array.length; i++) {
      +(function() {
        ...
        var _p = jump
          ? o.unshift.apply(o, unitArgs)
          : o.push.apply(o, unitArgs);
        parrs.push(_p);
      })();
    }

    var nextP = _Promise.defer();
    _Promise.all(parrs).then(
      function(data) {
        nextP.resolve(data);
      },
      function(err) {
